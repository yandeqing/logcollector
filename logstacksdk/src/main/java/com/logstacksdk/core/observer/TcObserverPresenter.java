package com.logstacksdk.core.observer;
...
import com.logstacksdk.core.TcStatSdk;
...
public class TcObserverPresenter implements TcNetworkObserver.INetworkListener, TcScreenObserver.IScreenListener,
    ...
    private synchronized void onForegroundChanged(Context aContext, boolean aIsForeground) {
        ...
        if (aIsForeground) {
            ...
            // app唤醒
            LogDataModel.storeAppAction(AppActionType.APPBACKGROUD);
            //切前台时开始计时
            scheduleStart();
    ...
    public void onNetworkConnected(Context aContext) {
        // 同步网络信息
        LogHeadUtil.getHeader(aContext).setNetworkinfo(LogHeadUtil.getNetWorkInfo(aContext));
        if (isForeground) {
            reportData(aContext);
            scheduleReStart();
        } else {
