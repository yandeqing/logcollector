...
public class TcStatSdk {
    ...
    private LogStaticsService staticsManager;
     ...
     * @param aContext
     *            context
     * @return 返回 LogStaticsService
     */
    public static synchronized TcStatSdk getInstance(Context aContext) {
        if (sInstance == null) {
            sInstance = new TcStatSdk(aContext,  new LogStaticsImpl(aContext));
        }
        return sInstance;
    }
     ...
     *            context
     */
    private TcStatSdk(Context aContext, LogStaticsService aStaticsManager) {
        mContext = aContext;
